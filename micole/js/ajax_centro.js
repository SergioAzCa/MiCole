...
var Servidor="Buscador_centro.php"; 
//var Zoom="XY.php";// Determina la pagina donde buscar
var Palabra="";              // Determina la ultima palabra buscada.

// funcion que realiza la conexion con el objeto XMLHTTP...
function Conectar() {
  if(window.XMLHttpRequest) Conexion=new XMLHttpRequest(); //mozilla
  else if(window.ActiveXObject) Conexion=new ActiveXObject("Microsoft.XMLHTTP"); //microsoft
}
...
function Solicitud(idContenido,Cadena) {
  ...
  if(Cadena && Cadena!=Palabra) {
	...
	Conexion.send("idContenido="+idContenido+"&campo="+Cadena+"&"+date.getTime());
    }else
...
function autocompletar2(idContenido,Cadena) {
...
function selectItem(idContenido,value,campo,x,y) {
  // alert("Ocultar");
  // Cuando pulsamos sobre el desplegable, colocamos el valor en el cuadro de texto
  var centro = value.replace("/", "'");
  document.getElementById("input2").value=centro;
