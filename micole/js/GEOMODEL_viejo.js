	...
	//MAPA
		...
		//CAPA CATASTRO
		 var Catastro = new ol.source.TileWMS({
			  url: 'http://13.94.208.26:8080/geoserver/wms?',
		...
		WMS_Catastro.setZIndex(2);
		 //CAPA PNOA
		  var PNOA = new ol.source.TileWMS({
			  url: 'http://13.94.208.26:8080/geoserver/wms?',
			  params: {'LAYERS': 'colegios_pnoa:OI.OrthoimageCoverage '},
			  serverType: 'geoserver',
			  crossOrigin: '*',
			});
		...
		WMS_PNOA.set('name', 'PNOA');
		 ...
		 // Herramientas
			...
			$(window).load(function() {
				...
				funcionBuscarEntrada(portal,calle,valor_entrada);
				var x_pos=$('#x_pos').val();
				var y_pos=$('#y_pos').val();
				console.log(x_pos)
			...
			$(document).ready(function(){
				...
				$(".btnInfo").click(function(){
					if (infoAct==true ){
						...
						map.removeLayer(buf_centro);
						overlay.setPosition(undefined);
						source.clear();	
						map.removeOverlay(overlay);
						map.getViewport().style.cursor = '';	
						url='';
						$(".btnInfo").removeClass("btnInfoActiva");
						
						infoAct = false;
					}else if(infoAct==false ){		
						...
						// Register a map click listener.
							...
							area_influencia_bach = new ol.layer.Tile({
							  source: new ol.source.TileWMS({
								url: 'http://13.94.208.26:8080/geoserver/colegios/wms?',
								params: params,
								serverType: 'geoserver',
								crossOrigin: '*',
							  })
...
function funcionBuscarEntrada(portal,calle,tipo){
			console.log(tipo)
				if (contador>0){
					vectorSource2.clear()
					iconFeatures2=[];
...
map.on('singleclick', function(evt) {
 if ($(".btnInfo").hasClass("btnInfoActiva")){
	  ...
	  if (capa=='buf_centro'){
		...
		$.ajax({
			...
			success:  function (response) {
				...
				content.innerHTML = "<div class='encabPopup'><span class='tituloPopUp'>Centro de enseñanza :</span></div><div class='contPopup'> - Nombre: "+response[0]+"<br/>- Teléfono: "+response[1]+"<br/>- Tipo: "+response[2]+"</div>";
...
/////////////////CENTROS
  ...
  url: function(extent, resolution, projection) {
    return 'http://13.94.208.26:8080/geoserver/wfs?service=WFS&' +
